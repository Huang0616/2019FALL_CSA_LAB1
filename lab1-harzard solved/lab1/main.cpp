...
//    newState.IF.nop = 0;
    ...
    while (1) {
        ...
        if(State.ID.nop == 1){          //nop propagation;
            ...
            //Decoder
            bitset<6> opcode = bitset<6> (State.ID.Instr.to_string().substr(0,6));
            ...
            if(opcode.to_ulong()==2){
                bitset<32> jumpaddr = bitset<32> (State.IF.PC.to_string().substr(0,4) + State.ID.Instr.to_string().substr(6,26)+"00");
                State.IF.PC = jumpaddr.to_ulong();
                newState.EX.nop = 1;
            }else{
